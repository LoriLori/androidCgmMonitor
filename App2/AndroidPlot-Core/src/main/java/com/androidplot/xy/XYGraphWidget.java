...
public class XYGraphWidget extends Widget {
    ...
    private void drawDomainTick(Canvas canvas, float xPix,float width, Paint linePaint) {
        float startY = domainSubTick ? gridRect.top : gridRect.bottom;
        float stopY = gridRect.bottom + domainLabelSubTickExtension;
        Paint tmp = new Paint(linePaint);
        tmp.setStyle(Paint.Style.FILL);
        tmp.setColor(Color.GRAY);
        tmp.setAlpha(80);

        float right =xPix + width;
        right =  right > gridRect.right ? gridRect.right : right;
        canvas.drawRect(xPix, startY, right, stopY, tmp);
    ...
    protected void drawGrid(Canvas canvas) {
        ...
        // draw ticks RIGHT of origin:
        {
            ...
            for (; xPix <= paddedGridRect.right; xPix = domainOriginF
                ...
                if (xPix >= paddedGridRect.left && xPix <= paddedGridRect.right) {
                    ...
                    if(i%2==0) {
                        drawDomainTick(canvas, xPix, domainStep.getStepPix(), domainGridLinePaint);
                    }
                }
                i++;
            }
        ...
        drawRangeGoodRange(canvas,rangeGridLinePaint, rangeStep.getStepPix());
    }

    private void drawRangeGoodRange(Canvas canvas, Paint rangeGridLinePaint, double stepPix) {
        float bottom = (float) (8.5*stepPix) + gridRect.top;
        float top = (float) (7*stepPix)  + gridRect.top;

        Paint paint = new Paint(rangeGridLinePaint);
        paint.setColor(Color.WHITE);
        paint.setStyle(Paint.Style.FILL);
        paint.setAlpha(40);
        canvas.drawRect(gridRect.left,top,gridRect.right,bottom,paint);
    }
